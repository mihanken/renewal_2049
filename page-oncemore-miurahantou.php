<?php
    /**
    * Template Name: oncemore-miurahantou page
    */
?>

<?php get_header(); ?>
<div class="hero__section works__hero-img">
  <h1 class="page-title">
    WORKS
  </h1>
</div>
<div class="works-single__wrapper">
  <div class="works-single__heading__flex-box">
    <div class="works-single__heading__title text">
      <h2>
        once more 三浦半島<br><span>/</span>MC三浦大根と野島ザファッカー
      </h2>
    </div>
  </div>
  <p class="works-single__heading__date">
    2019年3月27日公開
  </p>
  <div class="works-single__container">
    <div class="works-single__container__content">
      <iframe src="https://www.youtube.com/embed/WDSVN3bq1Rg" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen>
      </iframe>
    </div>
    <div class="works-single__container__txt text">
      <p>
        三浦半島研究会を卒業するメンバーに向けて、初の楽曲制作を行いました。
        <br>
        ロケ地は三浦半島全域。
      </p>
      <p>
        それぞれの土地の良さをマイメンに届けるべく、HIP-HOPに表現しました。
      </p>
    </div>
    <a href="https://mihanken.com/works" class="btn">
      < BACK
    </a>
  </div>
</div>
<?php get_footer(); ?>
