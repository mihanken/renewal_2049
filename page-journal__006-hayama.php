<?php
    /**
    * Template Name: journal__006-hayama page
    */
?>

<?php get_header(); ?>
<div class="hero__section works__hero-img">
  <h1 class="page-title">
    WORKS
  </h1>
</div>
<div class="works-single__wrapper">
  <div class="works-single__heading__flex-box">
    <div class="works-single__heading__title text">
      <h2>
        三浦半島ジャーナル006葉山号
      </h2>
    </div>
  </div>
  <p class="works-single__heading__date">
    2018年12月5日発行
  </p>
  <div class="works-single__container">
    <div class="works-single__container__content">
      <?php echo do_shortcode('[ngg src="galleries" ids="7" sortorder="270,284,283,282,281,280,279,278,277,276,277,276,275,274,273,272,271" display="basic_imagebrowser"]');?>
    </div>
    <div class="works-single__container__txt text">
      <p>
        三浦半島随一の観光の町、葉山。人はなぜ葉山に旅をしたくなるのか、そもそも人はなぜ旅をしたくなるのか。
      </p>
      <p>
        私たちは「旅」の在り方そのものに注意を向けて、葉山の旅の過ごし方を考えた。
      </p>
      <p>
        1000円ポッキリ貧乏旅行記、フェミニストが考える「葉山女子旅きっぷ」など掲載。
      </p>
    </div>
    <a href="https://mihanken.com/works" class="btn">
      < BACK
    </a>
  </div>
</div>
<?php get_footer(); ?>
